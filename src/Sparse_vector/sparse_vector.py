...
def from_slice_to_range(item):
    ...
    if isinstance(item, slice):
        return list(range(if_none(item.start, 0), item.stop))
    else:
        raise TypeError
...
def saver(sparse_vec, file_name):
    ...
    dump([sparse_vec.shape, sparse_vec.data, sparse_vec.indices], file_name)
...
def loader(file_name):
    ...
    [sparse_vec.shape, sparse_vec.data, sparse_vec.indices, sparse_vec.dtype] = data + [
        data[1].dtype
    ]
    return sparse_vec
...
class SparseVector:
    def __init__(self, arg1, shape=None, dtype=np.int64):
        ...
        if isinstance(arg1, int):
            ...
            if len(arg1) != 2:
                raise TypeError(
                    f"Can't use {arg1} as input, wrong number of elements to unpack."
                )
    ...
    def __getitem__(self, key):
        if isinstance(key, slice):
            ...
            if key.start < 0 or key.stop > self.shape:
                raise IndexError(
                    f"Can't use range {(key.start, key.stop)} in vector with {self.shape} shapegth"
                )
            ...
            if ind_left == ind_right:
                ...
                result = np.zeros(key.stop - key.start, dtype=self.data.dtype)
                start_ind = key.start

                for interval in range(ind_left, ind_right + 1):
                    if interval + 1 < self.indices.shape[0]:
                        result[
                            max(0, self.indices[interval] - start_ind) : self.indices[
                                interval + 1
                            ]
                            - start_ind
                        ] = self.data[interval]
                    else:
                        result[self.indices[interval] - start_ind :] = self.data[
                            interval
                        ]
                return result
    ...
    def __setitem__(self, key, value):
        ...
        if not (isinstance(value, np.ndarray) or isinstance(value, self.dtype)):
            raise TypeError(
                f"Wrong data type, vector has {self.dtype}, but tried to set {type(value)}"
            )
        ...
        if isinstance(value, np.ndarray):
            ...
            if isinstance(key, slice):
                ...
                indices = np.where(
                    np.diff(value, prepend=value[0] + 1, append=value[-1] + 1) != 0
                )[0]
            ...
            if isinstance(key, slice):
                ...
                if ind_left == ind_right:
                    ...
                    if ind_left_old == key.start and ind_right_old == key.stop:
                        ...
                        self.data = np.insert(
                            self.data, ind_left + 1, [value, self.data[ind_left]]
                        )
                        self.indices = np.insert(
                            self.indices, ind_left + 1, [key.start, key.stop]
                        )
                        self.tamp(ind_left + 1)
                        return
                    ...
                    if ind_left_old == key.start and ind_right_old == key.stop:
                        self.data[ind_left] = value
                        self.data = np.delete(
                            self.data, range(ind_left + 1, ind_right + 1)
                        )
                        self.indices = np.delete(
                            self.indices, range(ind_left + 1, ind_right + 1)
                        )
                        self.tamp(ind_left)
                        return
                        ...
                        self.indices = np.delete(
                            self.indices, range(ind_left + 1, ind_right)
                        )
                        self.tamp(ind_left)
                        return
                        ...
                        self.data = np.delete(
                            self.data, range(ind_left + 2, ind_right + 1)
                        )
                        self.indices = np.delete(
                            self.indices, range(ind_left + 2, ind_right + 1)
                        )
                        self.tamp(ind_left + 1)
                        return
                        ...
                        if ind_right - ind_left > 1:
                            ...
                            self.indices[ind_right] = key.stop
                            self.data = np.delete(
                                self.data, range(ind_left + 2, ind_right)
                            )
                            self.indices = np.delete(
                                self.indices, range(ind_left + 2, ind_right)
                            )
                        else:
                            self.indices[ind_right] = key.stop
                            self.data = np.insert(self.data, ind_left + 1, value)
    ...
    def __repr__(self):
        if self.shape <= 10:
            ...
            info += ", ..., "
            info += ", ".join(
                self.__getitem__(slice(self.shape - 6, self.shape)).astype(str)
            )
